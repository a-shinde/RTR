...
void initializeData()
{
    ...
    glGenBuffers(1, &vbo_position);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(squareCord), squareCord, GL_STATIC_DRAW);
    glVertexAttribPointer(POSITION_ATTRIB, 3, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(POSITION_ATTRIB);
    glBindBuffer(GL_ARRAY_BUFFER, 0);

    glVertexAttrib3f(COLOR_ATTRIB, 0.4f, 0.58f, 0.9f);
    //glEnableVertexAttribArray(COLOR_ATTRIB);
...
void Display()
{
    ...
    roatationMatrix = mat4::identity();
    modelViewMatrix = translate(1.5f, 0.0f, -6.0f);
    roatationMatrix = rotate(angleRotate, 1.0f, 0.0f, 0.0f);
    modelViewMatrix = modelViewMatrix * roatationMatrix;
