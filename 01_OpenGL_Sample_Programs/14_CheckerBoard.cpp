...
GLubyte checkerImage[checkerBoardHeight][checkerBoardWidth][4];
GLuint texture_checkerBoard;
void MakeCheckImage();

const GLfloat checkerBoardCord[] =
{
    0.0f, 1.0f, 0.0f,
    -2.0f, 1.0f, 0.0f,
    -2.0f, -1.0f, 0.0f,
...
const GLfloat checkerBoardCordTiled[] =
{
...
int LoadGLTextures()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, checkerBoardWidth, checkerBoardHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, checkerImage);
...
void resize(int width, int height)
{
    ...
    perspectiveProjectionMatrix = perspective(60.0f, (GLfloat)width / (GLfloat)height, 0.1f, 30.0f);
...
void Display()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(checkerBoardCord), checkerBoardCord, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);

    glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
    glBufferData(GL_ARRAY_BUFFER, sizeof(checkerBoardCordTiled), checkerBoardCordTiled, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
