...
HRESULT Initialize()
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "           float4 diffuse = Ld * Kd * max(dot(outVec.light_direction, outVec.tnorm), 0.0);"\
        "           float3 reflection_vector = reflect(-outVec.light_direction, outVec.tnorm);" \
        "           float3 viewer_vector = normalize(-eyeCoordinates.xyz);" \
        "           float4 specular = Ls * Ks * pow(max(dot(reflection_vector, outVec.viewer_vector), 0.0), shininess);" \
