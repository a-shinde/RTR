...
ID3D11Buffer *gpI_D3D11_Buffer_Cube_NormalBuffer = NULL;
...
HRESULT Initialize()
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "       float3 s = (float3)normalize(light_position - eyeCoordinates);" \
        "       outVec.diffuse_light = Ld * Kd * max(dot(s, tnorm), 0.0);" \
        "   }" \
        "   else" \
        "   {" \
        "       outVec.diffuse_light = float4(1.0, 1.0, 1.0, 1.0);" \
        "   }" \

        "   float4 worldCoord = mul(worldViewMatrix, pos);" \
    ...
    const char *pixelShaderSourceCode =
        "float4 main( float4 position : SV_POSITION, float4 diffuse_light : COLOR) : SV_TARGET"\
        "{ "\
        "   float4 outColor = diffuse_light;" \
        "   return (outColor);"\
        "}";
...
HRESULT initializeData()
{
    ...
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_Cube_NormalBuffer);
    ...
    // copy vertices into above buffer
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_Cube_NormalBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, cube_normals, sizeof(cube_normals));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_Cube_NormalBuffer, NULL);
...
void Display()
{
    ...
    gpI_D3D11_DeviceContext->IASetVertexBuffers(1, 1, &gpI_D3D11_Buffer_Cube_NormalBuffer, &stride, &offset);
    ...
    if (gbLight)
    {
        ...
        constantBuffer.light_position = XMVectorSet(0.0f, 0.0f, 2.0f, 1.0f);
