...
ID3D11Buffer *gpI_D3D11_Buffer_Pyramid_ColorBuffer = NULL;

ID3D11Buffer *gpI_D3D11_Buffer_Cube_PositionBuffer = NULL;
ID3D11Buffer *gpI_D3D11_Buffer_Cube_ColorBuffer = NULL;
...
HRESULT initializeData()
{
    ...
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_colors);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_Pyramid_ColorBuffer);
    ...
    // copy vertices into above buffer
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_Pyramid_ColorBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, pyramid_colors, sizeof(pyramid_colors));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_Pyramid_ColorBuffer, NULL);
    ...
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(cube_colors);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_Cube_ColorBuffer);
    ...
    // copy vertices into above buffer
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_Cube_ColorBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, cube_colors, sizeof(cube_colors));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_Cube_ColorBuffer, NULL);
...
void Uninitialize()
{
    ...
    if (gpI_D3D11_Buffer_Pyramid_ColorBuffer)
    {
        gpI_D3D11_Buffer_Pyramid_ColorBuffer->Release();
        gpI_D3D11_Buffer_Pyramid_ColorBuffer = NULL;
    }
...
void Display()
{
    ...
    gpI_D3D11_DeviceContext->IASetVertexBuffers(1, 1, &gpI_D3D11_Buffer_Pyramid_ColorBuffer, &stride, &offset);
    ...
    gpI_D3D11_DeviceContext->IASetVertexBuffers(1, 1, &gpI_D3D11_Buffer_Cube_ColorBuffer, &stride, &offset);
