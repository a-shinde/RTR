...
HRESULT Initialize()
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "       float3 tnorm = normalize(mul((float3x3)mul(worldMatrix, viewMatrix), (float3)normal));" \
        "       float3 light_direction = (float3)normalize(light_position - eyeCoordinates);" \
        "       float4 ambient = La * Ka;"\
        "       float4 diffuse = Ld * Kd * max(dot(light_direction, tnorm), 0.0);"\
        "       float3 reflection_vector = reflect(-light_direction, tnorm);" \
        "       float3 viewer_vector = normalize(-eyeCoordinates.xyz);" \
        "       float4 specular = Ls * Ks * pow(max(dot(reflection_vector, viewer_vector), 0.0), shininess);" \
        "       outVec.diffuse_light = ambient + diffuse + specular;" \
        "   }" \
        "   else" \
        "   {" \
