...
ID3D11Buffer *gpI_D3D11_Buffer_Triangle_PositionBuffer = NULL;
ID3D11Buffer *gpI_D3D11_Buffer_Triangle_ColorBuffer = NULL;
...
HRESULT initializeData()
{
    ...
    bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(triangle_positions);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_Triangle_PositionBuffer);
    ...
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_Triangle_PositionBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, triangle_positions, sizeof(triangle_positions));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_Triangle_PositionBuffer, NULL);

    //Copy Color Data
    ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
    bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(triangle_colors);
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_Triangle_ColorBuffer);
    ...
    // copy vertices into above buffer
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_Triangle_ColorBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, triangle_colors, sizeof(triangle_colors));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_Triangle_ColorBuffer, NULL);
...
void Uninitialize()
{
    ...
    if (gpI_D3D11_Buffer_Triangle_ColorBuffer)
    {
        gpI_D3D11_Buffer_Triangle_ColorBuffer->Release();
        gpI_D3D11_Buffer_Triangle_ColorBuffer = NULL;
    }

    if (gpI_D3D11_Buffer_Triangle_PositionBuffer)
    {
        gpI_D3D11_Buffer_Triangle_PositionBuffer->Release();
        gpI_D3D11_Buffer_Triangle_PositionBuffer = NULL;
    }
...
void Display()
{
    ...
    //Draw Triangle
    gpI_D3D11_DeviceContext->IASetVertexBuffers(0, 1, &gpI_D3D11_Buffer_Triangle_PositionBuffer, &stride, &offset);
    gpI_D3D11_DeviceContext->IASetVertexBuffers(1, 1, &gpI_D3D11_Buffer_Triangle_ColorBuffer, &stride, &offset);
    ...
    worldMatrix = XMMatrixRotationX(XMConvertToRadians(-angleRotate)) * XMMatrixTranslation(1.5f, 0.0f, 6.0f);
