...
ID3D11GeometryShader *gpI_D3D11_GeometryShader = NULL;
ID3D11Buffer *gpI_D3D11_Buffer_PositionBuffer = NULL;
ID3D11Buffer *gpI_D3D11_Buffer_ColorBuffer = NULL;
...
HRESULT Initialize()
{
    ...
    "void main(triangle outputVertex input[3], inout TriangleStream<Geometry_output> triStream)"\
    "{ "\
    "   Geometry_output output;" \
    "   for(uint index = 0; index < 3; index++)" \
    "   {" \
    "       output.position = mul(worldViewProjectionMatrix, (input[index].position + float4(0.0, 1.0, 0.0, 0.0)));" \
    "       output.color = input[index].color;" \
    "       triStream.Append(output);" \

    "       output.position = mul(worldViewProjectionMatrix, (input[index].position + float4(1.0, -1.0, 0.0, 0.0)));" \
    "       output.color = input[index].color;" \
    "       triStream.Append(output);" \
    ...
    ID3DBlob *pI_D3DBlob_GeometryShaderCode = NULL;
    pI_D3DBlob_Error = NULL;
    ...
    {
    outputFile.open(gszLogFileName, std::ofstream::app);
    outputFile << "D3DCompile() Failed For Geometry Shader : %s" << (char *)pI_D3DBlob_Error->GetBufferPointer() << endl;
    outputFile.close();
    pI_D3DBlob_Error->Release();
    pI_D3DBlob_Error = NULL;
    return(hr);
    ...
    {
    outputFile.open(gszLogFileName, std::ofstream::app);
    outputFile << "D3DCompile() Succeeded For Geometry Shader." << endl;
    outputFile.close();
    }

    hr = gpI_D3D11_Device->CreateGeometryShader(pI_D3DBlob_GeometryShaderCode->GetBufferPointer(), pI_D3DBlob_GeometryShaderCode->GetBufferSize(), NULL, &gpI_D3D11_GeometryShader);
    if (FAILED(hr))
    {
    outputFile.open(gszLogFileName, std::ofstream::app);
    outputFile << "ID3D11Device::CreateGeometryShader() Failed." << endl;
    outputFile.close();
    return(hr);
    }
    else
    ...
    gpI_D3D11_DeviceContext->GSSetShader(gpI_D3D11_GeometryShader, 0, 0);
    pI_D3DBlob_GeometryShaderCode->Release();
    pI_D3DBlob_GeometryShaderCode = NULL;
...
HRESULT initializeData()
{
    ...
    bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(positions);
    ...
    memcpy(mappedSubresource.pData, positions, sizeof(positions));
    ...
    bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
    bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = gpI_D3D11_Device->CreateBuffer(&bufferDesc, NULL, &gpI_D3D11_Buffer_ColorBuffer);
    ...
    // copy vertices into above buffer
    ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
    gpI_D3D11_DeviceContext->Map(gpI_D3D11_Buffer_ColorBuffer, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource);
    memcpy(mappedSubresource.pData, colors, sizeof(colors));
    gpI_D3D11_DeviceContext->Unmap(gpI_D3D11_Buffer_ColorBuffer, NULL);
    ...
    gpI_D3D11_DeviceContext->GSSetConstantBuffers(0, 1, &gpI_D3D11_Buffer_ConstantBuffer);
...
void Uninitialize()
{
    ...
    if (gpI_D3D11_Buffer_ColorBuffer)
    {
        gpI_D3D11_Buffer_ColorBuffer->Release();
        gpI_D3D11_Buffer_ColorBuffer = NULL;
    }

    if (gpI_D3D11_Buffer_PositionBuffer)
    {
        gpI_D3D11_Buffer_PositionBuffer->Release();
        gpI_D3D11_Buffer_PositionBuffer = NULL;
    }
...
void Display()
{
    ...
    gpI_D3D11_DeviceContext->IASetVertexBuffers(1, 1, &gpI_D3D11_Buffer_ColorBuffer, &stride, &offset);
    ...
    gpI_D3D11_DeviceContext->Draw(3, 0);
