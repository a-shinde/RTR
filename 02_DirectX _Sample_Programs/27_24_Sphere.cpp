...
struct CBUFFER
{
    ...
    XMVECTOR light_position[MAX_LIGHTS];
    unsigned int LKeyPressed;
    unsigned int lighting_type;
    XMVECTOR La[MAX_LIGHTS];
    XMVECTOR Ld[MAX_LIGHTS];
    XMVECTOR Ls[MAX_LIGHTS];
    XMVECTOR Ka;
    XMVECTOR Kd;
    XMVECTOR Ks;
    float shininess;
...
HRESULT Initialize()
{
    ...
    const char *vertexShaderSourceCode =
        ...
        "   float3 light_direction[MAX_LIGHT]: NORMAL2;" \
        "   float3 viewer_vector: NORMAL_VEC;" \
        "}; " \
        ...
        "   float4 light_position[MAX_LIGHT];" \
        "   uint LKeyPressed;" \
        "   uint lighting_type;" \
        "   float4 La[MAX_LIGHT];" \
        "   float4 Ld[MAX_LIGHT];" \
        ...
        "{   "\
        "   outputVertex outVec; " \
        "   outVec.diffuse_light = float4(0.0, 0.0, 0.0, 1.0);" \
        "   if (LKeyPressed == 1)" \
        "   {" \
        ...
        "       {" \
        "          outVec.light_direction[index] = (float3)normalize(light_position[index] - eyeCoordinates);" \
        "          if(lighting_type == 0)" \
        "          {" \
        "              float4 ambient = La[index] * Ka;"\
        "              float4 diffuse = Ld[index] * Kd * max(dot(outVec.light_direction[index], outVec.tnorm), 0.0);"\
        "              float3 reflection_vector = reflect(-outVec.light_direction[index], outVec.tnorm);" \
        "              float4 specular = Ls[index] * Ks * pow(max(dot(reflection_vector, outVec.viewer_vector), 0.0), shininess);" \
        "              outVec.diffuse_light = outVec.diffuse_light + ambient + diffuse + specular;" \
        "          }" \
        "      }" \
        "   }" \
    ...
    const char *pixelShaderSourceCode =
        ...
        "{ "\
        "   float4 phong_ads_color = float4(0.0, 0.0, 0.0, 1.0);" \
        "   if (LKeyPressed == 1)" \
        "   {" \
        "       if(lighting_type == 1)" \
        "       {" \
        ...
        "             float3 normalized_light_direction = normalize(input.light_direction[index]);" \
        "             float3 normalized_viewer_vector = normalize(input.viewer_vector);" \
        "             float4 ambient = La[index] * Ka;"\
        "             float4 diffuse = Ld[index] * Kd * max(dot(normalized_light_direction, normalized_tnorm), 0.0);"\
        "             float3 reflection_vector = reflect(-normalized_light_direction, normalized_tnorm);" \
        "             float4 specular = Ls[index] * Ks * pow(max(dot(reflection_vector, normalized_viewer_vector), 0.0), shininess);" \
        "             phong_ads_color = phong_ads_color + ambient + diffuse + specular;" \
        "          }" \
        "       }" \
        "      else" \
        "      {" \
        "           phong_ads_color = input.diffuse_light;" \
        "      }" \
        "   }" \
        "   else" \
        "   {" \
...
HRESULT initializeData()
{
    ...
    {
        constantBuffer.La[index] = XMVectorSet(light_ambient[0], light_ambient[1], light_ambient[2], light_ambient[3]);
        constantBuffer.Ld[index] = XMVectorSet(light_diffuse[0], light_diffuse[1], light_diffuse[2], light_diffuse[3]);
        constantBuffer.Ls[index] = XMVectorSet(light_specular[0], light_specular[1], light_specular[2], light_specular[3]);
        constantBuffer.light_position[index] = XMVectorSet(light_position[0], light_position[1], light_position[2], light_position[3]);
    }
...
void Display()
{
    ...
    XMMATRIX translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 1.6f);
    XMMATRIX rotationMatrix = XMMatrixRotationY(XMConvertToRadians(angleRotate));
    ...
    constantBuffer.lighting_type = lightingType;
    ...
    for (int row = 0; row < 6; row++)
    {
        ...
        {
            constantBuffer.Ka = XMVectorSet(materials[column * 6 + row][0], materials[column * 6 + row][1], materials[column * 6 + row][2], materials[column * 6 + row][3]);
            constantBuffer.Kd = XMVectorSet(materials[column * 6 + row][4], materials[column * 6 + row][5], materials[column * 6 + row][6], materials[column * 6 + row][7]);
            constantBuffer.Ks = XMVectorSet(materials[column * 6 + row][8], materials[column * 6 + row][9], materials[column * 6 + row][10], materials[column * 6 + row][11] );
            constantBuffer.shininess = materials[column * 6 + row][12];

            D3D11_VIEWPORT d3d_ViewPort;
            d3d_ViewPort.TopLeftX = (currentWidth * (float)column / 4);
            //d3d_ViewPort.TopLeftY = (currentHeight * (5 - row) / 6);
            d3d_ViewPort.TopLeftY = (currentHeight * (float)row / 6);
            d3d_ViewPort.Width = (float)currentWidth / 4;
            d3d_ViewPort.Height = (float)currentHeight / 6;
            d3d_ViewPort.MinDepth = 0.0f;
            d3d_ViewPort.MaxDepth = 1.0f;
            gpI_D3D11_DeviceContext->RSSetViewports(1, &d3d_ViewPort);

            // set orthographic matrix
            perspectiveProjectionMatrix = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0f), ((float)currentWidth / 4) / ((float)currentHeight / 6), 0.1f, 100.0f);
